...
def learn(env,
    ...
    for t in itertools.count():
        ...
        if np.random.rand(1) < exploration.value(t):
            ...
            action_val = session.run(Q_func, feed_dict={obs_t_ph: observation})
        ...
        if (t > learning_starts and
            ...
            # YOUR CODE HERE
            #print("########### IN TRAINING ###########")
            obs_t_batch, act_batch, rew_batch, obs_t1_batch, done_mask = replay_buffer.sample(batch_size)
            _ = session.run(train_fn, feed_dict={obs_t_ph: obs_t_batch, 
                                                 act_t_ph: act_batch,
                                                 rew_t_ph: rew_batch,
                                                 obs_tp1_ph: obs_t1_batch,
                                                 done_mask_ph: done_mask,
                                                 learning_rate: optimizer_spec.lr_schedule.value(t)})

            #print("Iteration learning") 
